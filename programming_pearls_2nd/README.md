...
## コラム2 「ああ、そうか！」アルゴリズム
* 二分探索
	* 与えられた40億個の32bit整数（重複なし）に含まれない32bit整数を1つ見つけよ
* 基本操作の繰り返し
	* 配列の左回転:abcdefghij →  defghijabc のアルゴリズムを考えよ
	* 1つずつずらす → 長さ x 回転分だけ走査するので遅い
	* お手玉 → メモリ消費が非常に少なく済むが、局所性が低いのでキャッシュが効きにくい
	* 反転の繰り返し → 再帰で解ける
* ソートで印をつける
	* 辞書からすべてのアナグラムを見つけること
	* ソートすることで、同種のもの（同値関係）を見つけ出せる
...
### 問題4
局所性というヒントから、多分、CPUのキャッシュ周りの話が関連していそう。
特定ブロックをキャッシュしたのに、すぐにキャッシュミスするのは、効率がメッチャ悪い。

### 問題5


### 問題6
キーの衝突がが0.2%程度であれば、`{ '5375*6*': ['LESK*M*'] }`のようなキーバリューのバリューを配列にしておいて、順番に聞いていく感じでいいのでは。
この場合、'5375*6*'が印になる。
